...
public class MainActivity extends Activity {
    ...
    void layoutUI() {
		...
		}
		{
			...
			// Change focus mode from any to manual to auto button
			view = findViewById(R.id.change_focus_mode);
			layoutParams = (RelativeLayout.LayoutParams)view.getLayoutParams();
			layoutParams.addRule(align_parent_top, RelativeLayout.TRUE);
			layoutParams.addRule(align_parent_bottom, 0);
			...
			layoutParams.addRule(align_parent_top, RelativeLayout.TRUE);
			layoutParams.addRule(align_parent_bottom, 0);
			layoutParams.addRule(left_of, R.id.change_focus_mode);
			layoutParams.addRule(right_of, 0);
			view.setLayoutParams(layoutParams);
			view.setRotation(ui_rotation);
	...
	public void clickedChangeFocusMode(View view) {
		...
		this.closePopup();
		String value_for_manual_focus = "focus_mode_manual2";
		//
		ImageButton changeFocusModeButton = (ImageButton) findViewById(R.id.change_focus_mode);
		String current_focus_mode = preview.getCurrentFocusValue();
		if(current_focus_mode != value_for_manual_focus){
			if( MyDebug.LOG )
				Log.d(TAG, "set focus to manual");
			changeFocusModeButton.setImageResource(R.drawable.focus_mode_auto);
			preview.updateFocus(value_for_manual_focus, false, true);
		}
		else if(current_focus_mode == value_for_manual_focus){
			if( MyDebug.LOG )
